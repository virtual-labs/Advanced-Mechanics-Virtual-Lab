...
function distanceOfBlade(scope) {
	...
	getChild("stand_right").x = CENTER + blade_distance * 4.5;  /** To set the postion of right side blade */
	getChild("stand_left").x = CENTER - blade_distance * 4.5;  /** To set the postion of left side blade */
	calculation();
	needlePositionSettings();
	stage.update();
}
...
function calculation(){
	/** Calculation of expression, e 𝑒=(𝑚𝑔𝑙^3)/(4𝑏𝑑^3 𝑌)	*/
	...
	pin_distance_from_equation = expression_e * 10;
	stage.update();
}
...
function resetExperiment(scope){
	...
	getChild("needle_zoom").y =  initial_pin_position;
	stage.update();
}